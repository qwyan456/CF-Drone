...
//
// 校准策略说明：
//   拆桨后电机空转，没有空气推力，IMU 无法检测力矩差异。所以地面拆桨校准不可行。
//   所有校准都必须在电机带桨运行时进行，利用 PID 积分量反推推力差异。
//
// 两级校准：
//   L1 粗调（motcal quick）：2秒快速采样，适合差异大、手动勉强能控的情况。
//        手持飞机离地30cm，输入命令后松手2秒即可。修正幅度大（每次最多15%）。
//   L2 精调（motcal hover）：5秒标准采样，适合能飞但漂移的情况。
//        悬停松杆5秒，修正幅度小（每次最多10%），精度更高。
//   两级可重复执行，逐步收敛。
//
// 原理：
//   悬停松杆后，如果某方向持续漂移，PID 内环 I 项会积分出补偿量。
//   这个积分量直接反映了各轴力矩的恒定偏差，按混控矩阵的逆变换分配到各电机即可。

// 通用校准核心：采样指定时长，根据 PID 积分算出补偿
void motcalCore(float duration, float maxCorrection, const char* label) {
	print("===== %s =====\n", label);

	if (!armed) {
		print("错误：请先解锁后再执行此命令\n");
		return;
	}
	if (mode != STAB) {
	...
	// 读取 PID 内环积分量
	float rollIntegral  = rollRatePID.integral;
	float pitchIntegral = pitchRatePID.integral;
	float yawIntegral   = yawRatePID.integral;
	...
	float rollCorrection  = constrain(rollIntegral  * rollRatePID.i  / avgThrust, -maxCorrection, maxCorrection);
	float pitchCorrection = constrain(pitchIntegral * pitchRatePID.i / avgThrust, -maxCorrection, maxCorrection);
	...
	print("可重复执行校准命令逐步收敛，直到悬停稳定。\n");
	print("===== 校准完成 =====\n");
}

// L1 粗调：2秒快速校准，修正幅度大，适合差异大时使用
void motcalQuick() {
	motcalCore(2.0f, 0.15f, "L1 粗调（快速校准）");
}

// L2 精调：5秒标准校准，修正幅度小，精度更高
void motcalHover() {
	motcalCore(5.0f, 0.10f, "L2 精调（悬停校准）");
}
